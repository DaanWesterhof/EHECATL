...
namespace EHECATL {
    ...
    void joystick::sendSticks() {
        int16_t temp_data[4];
        temp_data[0] = Read_ADC_Channel(&hadc, ADC_CHANNEL_3, 200) - (1830);
        temp_data[1] = Read_ADC_Channel(&hadc, ADC_CHANNEL_2, 200);
        temp_data[2] = Read_ADC_Channel(&hadc, ADC_CHANNEL_1, 200);
        temp_data[3] = Read_ADC_Channel(&hadc, ADC_CHANNEL_4, 200);
        bool equal = true;
        for (int i = 0; i < 4; i++) {
