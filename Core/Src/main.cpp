...
int main(void)
{
  ...
  EHECATL::communication<10> comms;
  EHECATL::controller<10> controll(htim2, comms);
  unsigned int data[2];
  ...
  while (_true)
  {
    ...
    /* USER CODE BEGIN 3 */
      ...
      data[0] = Read_ADC_Channel(&hadc1, ADC_CHANNEL_1, 100);
      data[1] = Read_ADC_Channel(&hadc1, ADC_CHANNEL_2, 100);
      comms.onMessageReceived(001, data);
      if(!HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_10)){
          if(!pressed) {
              comms.onMessageReceived(002, data);
              pressed = 1;
          }
